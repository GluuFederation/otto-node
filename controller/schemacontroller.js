...
exports.patchSchema = function (req, callback) {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return callback({
      error: ['Invalid Schema Id'],
      code: 400
  ...
  return schemaModel.findById(req.params.id)
    .then(function (schema) {
      if (!schema) {
        return Promise.reject({
          error: ['Schema doesn\'t exist'],
      ...
      if (req.body.op == 'add') {
        return common.patchAdd(req.body, schema);
      } else if (req.body.op == 'replace') {
        return common.patchReplace(req.body, schema);
      } else if (req.body.op == 'remove') {
        return common.patchRemove(req.body, schema);
      }
    })
    .then(function (oSchema) {
      return callback(null, oSchema);
